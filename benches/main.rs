...
use output_diffing_utility::{diff_text, diff_json, DiffConfig};
...
fn text_diff_benchmarks(c: &mut Criterion) {
    ...
    let medium_a: String = (1..=100).map(|i| format!("line {}", i)).collect::<Vec<_>>().join("\n");
    let medium_b: String = (1..=100).map(|i| {
        if i % 10 == 0 { format!("line {} modified", i) } else { format!("line {}", i) }
    }).collect::<Vec<_>>().join("\n");

    let large_a: String = (1..=1000).map(|i| format!("line {}", i)).collect::<Vec<_>>().join("\n");
    let large_b: String = (1..=1000).map(|i| {
        if i % 50 == 0 { format!("line {} modified", i) } else { format!("line {}", i) }
    }).collect::<Vec<_>>().join("\n");

    group.bench_function("small: 3 lines", |b| {
        b.iter(|| {
            black_box(diff_text(small_a, small_b, &DiffConfig::default()))
        })
    });

    group.bench_function("medium: 100 lines", |b| {
        b.iter(|| {
            black_box(diff_text(&medium_a, &medium_b, &DiffConfig::default()))
        })
    });

    group.bench_function("large: 1000 lines", |b| {
        b.iter(|| {
            black_box(diff_text(&large_a, &large_b, &DiffConfig::default()))
        })
    });
...
fn json_diff_benchmarks(c: &mut Criterion) {
    ...
    group.bench_function("small: 2 keys", |b| {
        b.iter(|| {
            black_box(diff_json(small_a, small_b, &DiffConfig::default()))
        })
    });

    group.bench_function("medium: nested objects", |b| {
        b.iter(|| {
            black_box(diff_json(medium_a, medium_b, &DiffConfig::default()))
        })
    });
...
criterion_group!(
    benches,
    text_diff_benchmarks,
    json_diff_benchmarks
);
